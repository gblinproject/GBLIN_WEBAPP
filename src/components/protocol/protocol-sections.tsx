              ...
              transactions.map((tx) => (
                <tr className="border-t border-white/5" key={tx.full_hash}>
                  <td className="px-5 py-4">
                    <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] ${tx.is_rebalance ? 'bg-amber-500/10 text-amber-300' : tx.type === 'BUY' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-sky-500/10 text-sky-300'}`}>
                      {tx.type}
                    </span>
                  </td>
    ...
    <div className="space-y-12">
      <section className={`${shellCard} p-7 sm:p-10`}>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard hint={t('dashboard.estimatedYield')} label={t('dashboard.navTitle')} loading={isOnChainLoading} value={onChainData?.nav || '$0.00'} />
          <MetricCard hint={t('dashboard.assetsInVault')} label={t('dashboard.tvlTitle')} loading={isOnChainLoading} value={formatCurrency(onChainData?.tvl || 0)} />
