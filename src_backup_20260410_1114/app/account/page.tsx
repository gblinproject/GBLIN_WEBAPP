...
import { ArrowLeft, Wallet, TrendingUp, TrendingDown, Send, Coins, LogOut, ExternalLink, Loader2, Copy, Check } from "lucide-react";
...
export default function AccountPage() {
  ...
  const handleTransfer = () => {
    ...
    const tx = prepareContractCall({
      ...
      params: [transferAddress as `0x${string}`, toWei(transferAmount, 18)],
    });
    sendTx(tx, {
      onSuccess: (data: any) => {
        showSuccess(data?.transactionHash ?? "");
  ...
  const tabs = [
    ...
    { key: "sell",     label: t("account.tabSell"),     icon: TrendingDown },
    { key: "send",     label: t("account.tabSend"),     icon: Send },
  ] as const;
                ...
                <PayEmbed
                  ...
                  payOptions={{
                    ...
                    transaction: prepareContractCall({
                      ...
                      params: [toWei((parseFloat(buyAmount) * 0.98).toString(), 18)],
                      value: toWei((parseFloat(buyAmount) * gblinPriceUsd / ethPriceUsd * 1.02).toString(), 18),
                    }),
                    metadata: { name: "GBLIN" },
                  }}
                  theme="dark"
        ...
        {activeTab === "sell" && (
          <div className={`${shellCard} p-6 sm:p-8`}>
            ...
            <div className="mx-auto max-w-md space-y-4">
              ...
              >
                {isSending ? <><Loader2 className="h-5 w-5 animate-spin" />{t("account.processing")}</> : <><TrendingDown className="h-5 w-5" />{t("account.sellBtn")}</>}
              </button>
            </div>
        ...
        {activeTab === "send" && (
          <div className={`${shellCard} p-6 sm:p-8`}>
            ...
            <div className="mx-auto max-w-md space-y-4">
              ...
              >
                {isSending ? <><Loader2 className="h-5 w-5 animate-spin" />{t("account.processing")}</> : <><Send className="h-5 w-5" />{t("account.sendBtn")}</>}
              </button>
            </div>
